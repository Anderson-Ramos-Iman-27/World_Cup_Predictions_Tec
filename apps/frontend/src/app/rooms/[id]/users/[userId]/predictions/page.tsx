...
export default function RoomUserPredictionsPage() {
  const params = useParams<{ id: string; userId: string }>();
  const router = useRouter();
  const [history, setHistory] = useState<RoomPredictionHistory | null>(null);
  ...
  useEffect(() => {
    apiRequest<RoomPredictionHistory>(`/rankings/rooms/${params.id}/users/${params.userId}/history`)
      .then(setHistory)
      .catch((error) =>
        setError(error instanceof Error ? error.message : 'No se pudo cargar el historial de la sala'),
      )
  ...
  return (
    <PrivateRoute>
      <AppShell
        title={history ? `${history.user.name} en ${history.room.name}` : 'Predicciones de sala'}
        subtitle="Revisa el historial de predicciones que hizo este participante dentro de la sala."
      >
          ...
          <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-3">
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        setNavigationMessage('Volviendo a la sala...');
                        router.back();
                      }}
                    >
                      Volver
                    ...
                    <Link
                      className="inline-flex h-10 items-center justify-center rounded-xl bg-action px-4 text-sm font-black text-white transition hover:bg-blue-700"
                      href={`/rooms/${history.room.id}/predictions`}
                      onClick={() => setNavigationMessage('Abriendo predicciones de la sala...')}
                    >
            ...
            {!isLoading && orderedPredictions.length === 0 ? (
              <div className="rounded-2xl border border-slate-200 bg-white p-6 text-sm text-slate-500 shadow-sm">
                Este participante aun no tiene predicciones registradas en esta sala.
              </div>
            ) : null}
