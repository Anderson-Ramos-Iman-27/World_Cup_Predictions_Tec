  ...
  return (
    <section
      className={`rounded-2xl border p-4 transition ${
        active ? 'border-action bg-blue-50' : 'border-slate-200 bg-white'
      }`}
    >
      <button
      ...
      >
        <span className="flex min-w-0 gap-3">
          <span
            className={`mt-1 h-4 w-4 shrink-0 rounded-full border ${
              active ? 'border-action bg-action shadow-[inset_0_0_0_4px_white]' : 'border-slate-300 bg-white'
            }`}
          />
          <span>
        ...
        <span className="flex shrink-0 flex-col items-end gap-1">
          <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-action shadow-sm">
            {points}
          </span>
          {isSubmitted ? (
